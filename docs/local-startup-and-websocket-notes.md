...
- `http://127.0.0.1:8001/api/chart/bootstrap` 可访问；
- `ws://127.0.0.1:8001/ws/chart/session-local` 可建立连接；
...
```powershell
$env:TRADINGASSISTANT_API_BASE_URL='http://127.0.0.1:8001'
...
- Reflex 内部后端运行在 `http://127.0.0.1:8002`
- `http://127.0.0.1:8002/ping` 返回 `"pong"`
- `http://127.0.0.1:8002/_health` 返回 `{"status": true}`
...
- Reflex 内部连接：`ws://localhost:8002/_event`
- 业务图表连接：`ws://127.0.0.1:8001/ws/chart/session-local`
...
- `http://127.0.0.1:8001/api/chart/bootstrap?...` 返回 `200`
- `http://127.0.0.1:8002/ping` 返回 `200`
- `http://127.0.0.1:8002/_health` 返回 `200`
- `ws://127.0.0.1:8001/ws/chart/session-local` 收到 `subscription_ack`
- `ws://localhost:8002/_event` 可通过 Socket.IO 成功建立连接
...
- `8001` 的业务后端是否已启动；
- `TRADINGASSISTANT_API_BASE_URL` 是否指向 `http://127.0.0.1:8001`；
- `http://127.0.0.1:8001/api/chart/bootstrap` 是否能返回 `200`。

### 7.2 页面提示连接异常

优先检查：

- `8002` 的 Reflex 内部后端是否已启动；
- `http://127.0.0.1:8002/ping` 是否返回 `pong`；
- `http://127.0.0.1:8002/_health` 是否返回 `{"status": true}`。

### 7.3 端口冲突

如果 `3000`、`8001`、`8002` 中任一端口被占用，先释放冲突进程，或统一修改配置后再启动。